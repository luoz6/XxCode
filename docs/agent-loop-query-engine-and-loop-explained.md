...
这张图只保留了最核心的闭环，并把恢复判断放在 `assistant turn` 提交之前。后面无论是上下文压缩、memory 注入还是 skills，基本都可以视为对这条主循环的增强，而不会改变它的基本结构。
...
每一轮循环开始时，系统都会基于当前的 `state` 重新组织一份要发送给模型的请求。真正发起请求之前，`while True` 顶部还会先检查是否需要通过 context pipeline 压缩上下文，并完成必要的注入和消息规范化；这里先聚焦最终效果：这一轮模型看到的输入，已经包含了此前所有被提交进消息历史的内容。
...
这里可以注意两点。第一，发送给模型的不是裸的用户输入，而是 `state.messages` 经整理、修正后的结果。第二，这一轮请求里会附带当前可用工具的 schema，所以模型不只是“回答问题”，还可以在回答过程中发出工具调用。
...
如果没有工具调用，说明模型认为在当前上下文下已经可以直接收束任务了，那么整个循环就到这里结束。真实实现里，在产出 cost 和 done 之前，loop 还会按配置调度一次后台 memory extraction，趁本轮对话刚结束时把可能有价值的信息沉淀到持久记忆里。反过来，只要还有工具调用，这个闭环就会继续往下推进。
...
这六步已经足够勾出 agent-loop 最核心的运行方式。下面三节（6-8）分别展开步骤 2、步骤 3+6、步骤 6，讲清楚一轮输出如何从流式碎片变成下一轮的输入。
...
这里的 `role="user"` 不是随手选的，它对应的是这套消息格式里“把工具执行结果重新喂回模型”的那一侧。也就是说，assistant 先提出 `tool_use`，系统再把执行结果作为新的输入消息送回去。
...
所以第 8 节更关心的不是“什么时候写入”，而是“为什么写入以后就能驱动下一轮”。关键点有两个：第一，`tool_result` 进入的是消息历史，而不是某个只给 UI 展示的执行缓存；第二，它以 `role="user"` 的消息形态进入历史，等价于把外部世界刚刚返回的观察结果重新送回模型输入侧。

在 [messages.py](/f:/agent/XxCode/XxCode/src/xxcode/agent/messages.py:259) 中，这一步最终会追加一条新的消息：
...
loop 顶部下一次发起模型请求时，`_build_messages(state)` 会从这份更新后的 `state.messages` 里组装输入。模型看到的因此不只是原始用户问题，也不只是上一轮 assistant 的输出，而是一个更完整的上下文序列：
...
除了模型自然收敛之外，loop 还有一个硬性的 `max_parent_turns` 上限。当 `state.turn_count` 达到这个阈值时，循环会主动终止并返回错误，防止任务在工具调用中无限循环。
...
所以这些逻辑不能放在 UI 层，也不能等循环结束以后再统一补救。它们必须长在 loop 里，而且必须长在一个非常具体的位置上：本轮模型输出已经收集完成、工具是否调用也已经可见，但 `assistant turn` 还没有正式提交进消息历史之前。
...
这个顺序很关键。系统不是先把有问题的输出写进历史，再看能不能补救，而是先判断这一轮是否适合进入正式历史，或者是否应该先恢复后再继续。
...
- 当前输入是不是 skill 调用
- 本轮结束后 cost 怎么记录
- 是否读取或保存 `core_engine._last_state`，供下次对话复用
...
首先是上下文工程。它会影响每一轮发给模型的输入长什么样，包括消息规范化、上下文压缩、缓存断点、某些运行时注入等。在主循环里，它的位置其实已经很明确：每轮 `while True` 开始后、真正发起模型请求之前，loop 会先判断当前消息是否超过压缩阈值，并在需要时调用 context pipeline。但这些内容更像是在优化“这一轮模型看到什么”，而不是在改变循环本身的骨架。

其次是 memory。当前实现里，memory index 注入、recalled memories 预取、工具后追加 fresh recall、循环自然退出时的后台 extraction 这些能力都已经接在 loop 周围了。尤其是 extraction：当模型不再调用工具、主循环即将收束时，系统会调度一次后台提取，把本轮对话中可能长期有用的信息沉淀下来。但如果在本文里展开它们，会很容易把主线从“循环如何推进任务”带偏到“记忆系统如何增强模型输入”。
